; 这是一个为ExcelToJson编写的可视化UI,整合了错误日志打印、可视化进度、目录和配置管理等功能
...
global isTop:=0, config_obj:="", config_json:="", xlsx_path:="", json_path:="", exportmode:=0, progressbar:=0
    , default_config:="
(
{
    ""xlsx_path"" : """ A_ScriptDir """,
    ""json_path"" : """ A_ScriptDir """,
    ""exportmode"" : 0
}
)"

Gui, 主界面:new, -Resize +Hwndid, Excel To Json
Gui, 主界面:Default
Gui, Color, F0F0F0
Menu, 程序_菜单项, Add, 保持在最上方, 保持在最上方
Menu, 程序_菜单项, Add, 打开错误日志, 打开错误日志
Menu, 程序_菜单项, Add, 清空错误日志, 清空错误日志
Menu, 程序_菜单项, Add, 退出程序, 退出程序
Menu, 设置_菜单项, Add, 恢复默认设置, 恢复默认设置
Menu, 关于_菜单项, Add, 关于此程序, 关于
Menu, 菜单栏, Add, 程序, :程序_菜单项
Menu, 菜单栏, Add, 设置, :设置_菜单项
Menu, 菜单栏, Add, 关于, :关于_菜单项
Gui, Menu, 菜单栏

Gui, Add, text, Section xm ym+5, xlsx文件所在路径:
Gui, Add, text, x+2 yp-3 w200 h18 Border +0x8000 vxlsx_path,
Gui, Add, Button, x+0 yp-3 w25 g浏览xlsx, ...
Gui, Add, Button, x+10 g打开xlsx目录, 打开
Gui, Add, text, xs yp+40, json文件存放路径:
Gui, Add, text, x+2 yp-3 w200 h18 Border +0x8000 vjson_path,
Gui, Add, Button, x+0 yp-3 w25 g浏览json, ...
Gui, Add, Button, x+10 g打开json目录, 打开
Gui, Add, text, xs yp+40, exportmode:
Gui, Add, DDL, x+2 yp-3 w30 vexportmode Choose%exportmode% g设置导出模式, 0|1|2|3
Gui, Add, text, x+5 yp+3 w270 r2 vexportmode注释,
Gui, Add, Progress, xs yp+40 w260 h20 -smooth vprogressbar, 0
Gui, Add, Button, xs+275 yp-2 w100 g开始导出, 开始导出
Gui, Add, StatusBar,,
SB_SetParts(270)
gosub, 读取配置
Gui, Show, w400 h170, Excel To Json
SetTimer, statusBar_update, 1000
return
...
保持在最上方:
    ...
    Menu, 程序_菜单项, ToggleCheck, 保持在最上方
return

打开错误日志:
    if(FileExist(A_ScriptDir . "\errorlog.log"))
        Run, %A_ScriptDir%\errorlog.log
    else
        MsgBox,,, 文件不存在, 5
return

清空错误日志:
    FileDelete, %A_ScriptDir%\errorlog.log
    FileAppend,, %A_ScriptDir%\errorlog.log
return
...
关于:
    Gui, 关于:new, -SysMenu, 关于此程序
    Gui, 关于:Add, text,, 本程序是为ExcelToJson库编写的可视化UI
    Gui, 关于:Add, link,, 项目的GitHub地址是:<a href="https://github.com/liuchengsis/ExcelToJson">https://github.com/liuchengsis/ExcelToJson</a>
    Gui, 关于:Add, text,, 若有反馈意见可以发issue,或者联系作者QQ441066542
    Gui, 关于:Add, Button, Default w30 h20 g关闭关于窗口, OK
    Gui, 关于: +owner主界面
    Gui, 关于:Show
return
...
读取配置:
    if(FileExist(A_ScriptDir . "\config.json"))
        FileRead, config_json, %A_ScriptDir%\config.json
    else
        config_json:=default_config, create_jsonfile(A_ScriptDir,"config",ahk_tojson(json_toobj(default_config)))
    config_obj:=json_toobj(config_json), xlsx_path:=config_obj.xlsx_path, json_path:=config_obj.json_path, exportmode:=config_obj.exportmode
    GuiControl,, xlsx_path, %xlsx_path%
    GuiControl,, json_path, %json_path%
    GuiControl, ChooseString, exportmode, %exportmode%
    gosub,更新注释
return

浏览xlsx:
    Gui +OwnDialogs
    FileSelectFolder, xlsx_path, *%xlsx_path%, 4, 选择文件夹
    config_obj.xlsx_path:=xlsx_path, create_jsonfile(A_ScriptDir,"config",ahk_tojson(config_obj))
    GuiControl,, xlsx_path, %xlsx_path%
return

打开xlsx目录:
    if(FileExist(xlsx_path))
        Run, explorer %xlsx_path%
    else
        MsgBox,,, 路径不存在, 5
return
...
浏览json:
    Gui +OwnDialogs
    FileSelectFolder, json_path, *%json_path%, 4, 选择文件夹
    config_obj.json_path:=json_path, create_jsonfile(A_ScriptDir,"config",ahk_tojson(config_obj))
    GuiControl,, json_path, %json_path%
return

打开json目录:
    if(FileExist(json_path))
        Run, explorer %json_path%
    else
        MsgBox,,, 路径不存在, 5
return

设置导出模式:
    GuiControlGet, exportmode
    config_obj.exportmode:=exportmode+0, create_jsonfile(A_ScriptDir,"config",ahk_tojson(config_obj))
    gosub, 更新注释
return
...
更新注释:
    ...
    {
    Case 0:
        exportmode注释:="只导出合并数据后的主表"
    Case 1:
        exportmode注释:="导出合并数据后的主表和含主键的从表(合并数据后的从表数据中不存在外键)"
    Case 2:
        exportmode注释:="导出合并数据后的主表和所有原始从表(从表的外键也会保留)"
    Case 3:
        exportmode注释:="不合并数据，强制导出所有原始表"
    }
    GuiControl,, exportmode注释, %exportmode注释%
return

开始导出:
    GuiControl,, progressbar, 0
    excel_tojson(xlsx_path,json_path,exportmode)
return
...
; 更新进度条
progress_update(pos_progress)
{
    GuiControl,, progressbar, %pos_progress%
    return
}

; 更新状态栏
statusBar_update:
    FormatTime, times, A_Now, yyyy-MM-dd HH:mm:ss
    Gui, 主界面:Default
    SB_SetText(times,2)
return
...
excel_tojson(xlsx_path,json_path,exportmode:=0)
{
    ...
    dest:=json_path, xlsxlist:=[], starttime:=A_TickCount
    ...
    key_main_list:="0", sheetname:="", parent_name:="", master_table:={}, follower_table_haskey:={}, follower_table_nokey:={}, pre_progress:=0, late_progress:=0
    for k, v in xlsxlist
    {
        ...
        loop % wb.worksheets.Count
        {
            ...
            if SubStr(ws.Name,1,1)="!"
                continue
            SB_SetText("正在解析" . wb.Name . "/" . ws.Name)
        ...
        wb.Close(), wb:="", pre_progress++, progress_update(Ceil(50*pre_progress/xlsxlist.Count()))
    }
    ; 先备份缓存，以便依据不同模式导出
    master_table_cache:={}, follower_table_nokey_cache:={}, follower_table_haskey_cache:={}, count:=0
    for k, v in master_table
        master_table_cache[k]:=ahk_tojson(v), SB_SetText("缓存" . k . ".json")
    for k, v in follower_table_nokey
        follower_table_nokey_cache[k]:=ahk_tojson(v[1]), SB_SetText("缓存" . k . ".json")
    for k, v in follower_table_haskey
        follower_table_haskey_cache[k]:=ahk_tojson(v[1]), SB_SetText("缓存" . k . ".json")
    ; 从缓存中取出对象，依据引用关系重组对象后导出json
    for k, v in follower_table_nokey ;k->sheetname, v->[ahk_obj.Clone(),parent_name,0]
    {
        if(v[3]==0)
        {
            SB_SetText("合并:表" . k . "--->" . v[2])
            ...
            else if(master_table.HasKey(v[2])) ;再查找主表集合
            {
                ...
                v[3]:=-1, SB_SetText("表" . k . "合并失败，找不到父表"), errlog("Table_Error_NoMaster",k,v[2])
        }
    }
    for k, v in follower_table_haskey ;k->sheetname, v->[ahk_obj.Clone(),parent_name,0]
    {
        ...
        if(v[3]==0 && follower_table_haskey.HasKey(v[2])) ;带主键的从表先在同类中查找父表
        {
            if(follower_table_haskey[v[2]][2]!=k)
            {
                ...
                v[3]:=-1, SB_SetText("表" . k . "合并失败，循环引用"), errlog("Table_Error_Circular_Reference",k,v[2])
        }
    ...
    {
        total:=master_table.Count()
        for k, v in master_table
            SB_SetText("导出" . k . ".json"), count++, s:=ahk_tojson(v), create_jsonfile(dest,k,s), late_progress++, progress_update(Ceil(50+50*late_progress/total))
    }
    else if(exportmode==1) ;导出合并数据后的1类和3类表
    {
        total:=master_table.Count() + follower_table_haskey.Count()
        for k, v in master_table
            SB_SetText("导出" . k . ".json"), count++, s:=ahk_tojson(v), create_jsonfile(dest,k,s), late_progress++, progress_update(Ceil(50+50*late_progress/total))
        for k, v in follower_table_haskey
            (v[3]==1 && (SB_SetText("导出" . k . ".json"), count++, s:=ahk_tojson(v[1]), create_jsonfile(dest,k,s), late_progress++, progress_update(Ceil(50+50*late_progress/total))))
    }
    else if(exportmode==2) ;导出合并数据后的1类表和所有原始2类3类表
    {
        total:=master_table.Count() + follower_table_haskey_cache.Count() + follower_table_nokey_cache.Count()
        for k, v in master_table
            SB_SetText("导出" . k . ".json"), count++, s:=ahk_tojson(v), create_jsonfile(dest,k,s), late_progress++, progress_update(Ceil(50+50*late_progress/total))
        for k, v in follower_table_haskey_cache
            SB_SetText("导出" . k . ".json"), count++, create_jsonfile(dest,k,v), late_progress++, progress_update(Ceil(50+50*late_progress/total))
        for k, v in follower_table_nokey_cache
            SB_SetText("导出" . k . ".json"), count++, create_jsonfile(dest,k,v), late_progress++, progress_update(Ceil(50+50*late_progress/total))
    }
    else if(exportmode==3) ;不导出合并数据后的表，强制导出所有原始表
    {
        total:=master_table_cache.Count() + follower_table_haskey_cache.Count() + follower_table_nokey_cache.Count()
    ...
    Process, Close, %p_pid%
    SB_SetText("导出完成,共导出 " . count . " 个json文件,耗时 " . ElapsedTime . " ms")
    return
}
...
; 错误日志打印
errlog(errcode,param1:="",param2:="",param3:="",param4:="")
{
    FormatTime, t, A_Now, yyyy-MM-dd HH:mm:ss
    Switch errcode
    {
    Case "Type_Error_Boolean":
        newlog:= t . "=>" . errcode . ":布尔类型错误`n位于" . param1 . "表" . param2 . "行" . param3 . "列" . param4 . "`n"
    Case "Table_Error_NoMaster":
        newlog:= t . "=>" . errcode . ":表引用错误`n表" .  param1 . "找不到父表" . param2 . "`n"
    Case "Table_Error_Circular_Reference":
        newlog:= t . "=>" . errcode . ":表引用错误`n表" .  param1 . "与表" . param2 . "存在循环引用`n"
    }
